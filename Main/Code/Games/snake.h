...
// moving/updating functions
    void moveSnake(List* snakeBodyPtr, enum Moving* moving, uint8_t xHead, uint8_t yHead, uint8_t* lastXPtr, uint8_t* lastYPtr); // updates snake xVel and yVel; then it pushes the tail to the head (updating cords)
...
// draw functions
    ...
    void drawSnakeCell(uint8_t x, uint8_t y); // draws a single snake body cell
    void drawSnakeFull(List snakeBody); // draws the full snake body
    void clearSnake(uint8_t tailx, uint8_t taily, bool* lightGreenCell); // clears the old background cell after snake movements
